  ...
  >
      ...
      <div className="w-[44%] sm:w-[55%] lg:flex-1 py-3 pr-3 sm:py-4 sm:pr-4 lg:py-6 lg:pr-6 lg:pl-0 flex flex-col justify-between h-full lg:h-[320px]">
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <h3 className="text-[#222] font-medium lg:font-bold text-[13px] sm:text-[15px] lg:text-2xl leading-[1.3] line-clamp-5 lg:line-clamp-3 mb-1 lg:mb-2">
            {product.name}
          </h3>

          <p className="text-[#888] text-[10px] sm:text-[11px] lg:text-sm capitalize mb-1 lg:mb-2 lg:tracking-wide lg:font-medium">{product.brand}</p>

          <p className="hidden lg:block text-[#666] text-sm leading-relaxed line-clamp-2 mb-4">
            {product.description}
          </p>
        ...
        <div className="hidden lg:flex items-center gap-4 mt-auto">
          <div className="flex items-center gap-2.5 w-1/2 flex-shrink-0">
            <span className="text-[#e02b2b] font-bold text-3xl">
              {product.price.toFixed(1)} Bs
            </span>
            {product.originalPrice && <span className="text-[#999] text-lg line-through">
                {product.originalPrice.toFixed(1)} Bs
              </span>}
          </div>
          <Button
